...
/* returns the shortened url */
function get_bitly_lookup_url($url,$login='sdbruder',$appkey='R_5257924e91d9cf6154f1dfa347433067',$format='txt') {
        $connectURL = 'https://api.bit.ly/v3/user/link_lookup?login='.$login.'&apiKey='.$appkey.'&url='.urlencode($url).'&format='.$format;
        return curl_get_result($connectURL);
}
