...
function draw_cg(context,zoom,p,wholewing) {
    ...
    var cg_radius = Math.max(1.5*panels[0].span/100, 5);
...
function load_panels(qty) {
    ...
    for(i=1;i<=qty;i++) {
        ...
        if (p.span>0) {
            ...
            // MAC length & distance
            // window.panels[i].maclen =  p.root - (2 * (p.root - p.tipc) * (0.5 * p.root + p.tipc) / (3 * (p.root + p.tipc)));
            window.panels[i].maclen = p.te_mac_y - p.le_mac_y;
            /*
            if (p.root != p.tipc) {
                window.panels[i].macdist = p.span * ((p.root - p.maclen) / (p.root - p.tipc));
            } else {
                window.panels[i].macdist = p.span / 2;
            }*/
